...
module.exports = logTest(
  ...
  async function (context, commands) {
    context.log.info("Profiling Speedometer 3 Workloads with ETW");
    ...
    const expose_profiler = context.options.browsertime.expose_profiler;
    ...
    for (const suite of suites) {
      ...
      await logTask(context, `Suite: ${suite}`, async function () {
        ...
        let data = await commands.js.run(`
          ...
          return { score, values: values.formattedMean };
        `);
        if (expose_profiler === "true") {
          if (context.options.browser === "firefox") {
