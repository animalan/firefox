...
class ProfileSymbolicator:
    ...
    def symbolicate_profile(self, profile_json):
        ...
        profiler_edit_path = Path(
            "/Users/animalan/profiler/node-tools-dist/profiler-edit.js"
        )
        ...
        if not self._validate_symbolication_deps([
            ...
            LOG.info(
                f"Symbolication dependencies not found."
                f"{'Run ./mach bootstrap to install them.' if not in_ci else ''}"
            )
            return
        ...
        try:
            ...
            if not Path(breakpad_symbol_dir).exists():
                LOG.critical(
